// Numeric value for each rank character
const RANK_VAL = {
...
// Parse a card string like "Ah" or "10s" into { rank, suit, str }
...
// All k-length combinations from arr
...
function evaluate5(cards) {
  ...
  if (isStraight && isFlush) {
    ...
    score = handRank * 1_000_000 + straightHigh;
  } else if (groups[0].count === 4) {
    handRank = 7; name = "Four of a Kind";
    score = handRank * 1_000_000 + groups[0].rank * 100 + groups[1].rank;
  } else if (groups[0].count === 3 && groups[1].count === 2) {
    handRank = 6; name = "Full House";
    score = handRank * 1_000_000 + groups[0].rank * 100 + groups[1].rank;
  } else if (isFlush) {
    handRank = 5; name = "Flush";
    score = handRank * 1_000_000 + ranks[0] * 10000 + ranks[1] * 1000 + ranks[2] * 100 + ranks[3] * 10 + ranks[4];
  } else if (isStraight) {
    handRank = 4; name = "Straight";
    score = handRank * 1_000_000 + straightHigh;
  } else if (groups[0].count === 3) {
    handRank = 3; name = "Three of a Kind";
    score = handRank * 1_000_000 + groups[0].rank * 10000 + groups[1].rank * 100 + groups[2].rank;
    ...
    score = handRank * 1_000_000 + hi * 10000 + lo * 100 + groups[2].rank;
  } else if (groups[0].count === 2) {
    handRank = 1; name = "Pair";
    score = handRank * 1_000_000 + groups[0].rank * 10000 + groups[1].rank * 100 + groups[2].rank * 10 + groups[3].rank;
  } else {
...
export function evaluateHand(holeCards, boardCards) {
